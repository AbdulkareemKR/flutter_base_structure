...
Future<List<Timeslot>> getAvailableTimeslots(String serviceId) async {
  ...
  if (timeslotsDocs.docs.isNotEmpty) {
    final timeslotsList = timeslotsDocs.docs.map((timeslot) => Timeslot.fromMap(timeslot.data())).toList();
...
Map<String, List<Timeslot>> groupTimeslotsByDay(Iterable<dynamic> timeslots, String localCode) {
  ...
  for (final timeslot in timeslots) {
    final day = DateFormat('EEEE', localCode).format(timeslot.dateFrom.toDate()) +
        " " +
        DateFormat('Md', localCode).format(timeslot.dateFrom.toDate());
...
Stream<List<Timeslot>> getTimeslotsStream(String serviceProviderId, {DateTime? date}) {
  try {
