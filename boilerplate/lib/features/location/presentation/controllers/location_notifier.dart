...
import 'package:garage_client/features/location/domain/models/location_state.dart';
...
import 'package:garage_client/services/neighborhood_repo.dart';
...
import 'package:garage_client/services/random_generator.dart';
...
class LocationNotifier extends StateNotifier<LocationState> {
  ...
  Future<void> onLocationSavePress({required String uid, required BuildContext context}) async {
    ...
    if (!Validator.safeIsNotEmpty(noteController.text)) {
      ...
      final userNeighborhood = await ref.read(nearestNeighborhoodProvider(latLng).future);

      if (userNeighborhood == null) {
        await GarageDialog.show(
            context: context, style: DialogStyle.error, message: 'location.locationChosenNotSupported'.translate());
      } else {
