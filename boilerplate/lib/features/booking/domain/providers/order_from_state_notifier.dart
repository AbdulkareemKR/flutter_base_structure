...
import 'package:garage_client/models/async_response/async_response.dart';
...
import 'package:garage_client/models/locale_response/locale_response.dart';
    ...
    ref.watch(subServicesProvider(serviceId)).when(data: ((services) async {
      try {
        ...
        final nearestTimeslot =
            await ref.read(timeslotsRepoProvider).getNearestTimeslot(services: services,  carOwner: carOwner);

        final Timeslot? timeslot;
        final double totalPrice;
        if ((nearestTimeslot.timeslots.isNotEmpty)) {
          timeslot = nearestTimeslot.timeslots.first;
        } else {
          timeslot = null;
        }
        if (timeslot != null) {
          totalPrice = (await getServicePriceForServiceProvider(nearestTimeslot.service.id, timeslot.serviceProviderId,
                  ref.watch(carOwnerProvider).value?.defaultCar?.carId))
              .toDouble();
        } else {
          totalPrice = 0.0;
        }
  ...
  Future<void> updateTotalPrice() async {
    ...
    state.whenData((orderInfo) async {
      ...
      if (orderInfo.timeslot != null) {
        totalPrice += (await getServicePriceForServiceProvider(orderInfo.selectedService!.id,
